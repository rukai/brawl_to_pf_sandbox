...
fn main() {
    let export_fighter_list: Vec<String> = env::args().map(|x| x.to_lowercase()).collect();
    ...
    match fs::read_dir("data/brawl/fighter") {
        Ok(fighter_dir) => {
            ...
            for brawl_fighter in brawl_fighters {
                if export_fighter_list.contains(&brawl_fighter.cased_fighter_name.to_lowercase()) || export_fighter_list.contains(&String::from("all")) {
